...
class SimpleMDDriver(DocumentDriver):
    ...
    def make(self):
        with open(self.input_path, "r") as input_file:
            ...
            is_in_frontmatters: bool = False
            try:
                iteration_count = 0
                for pair in enumerate(lines):
                    (line_index, each_line) = pair
                    was_in_code_block = False
                    was_in_frontmatters = False
                    if line_index < len(self.translated_lines):
                        continue
                    if each_line.startswith('```'):
                        was_in_code_block = is_in_code_block
                        is_in_code_block = not is_in_code_block
                    if each_line.startswith('---'):
                        was_in_frontmatters = is_in_frontmatters
                        is_in_frontmatters = not is_in_frontmatters
                    is_empty_line = len(each_line.strip()) == 0
                    is_image = each_line.startswith('![')
                    if is_in_code_block or was_in_code_block or is_in_frontmatters or was_in_frontmatters or is_empty_line or is_image:
                        self.translated_lines.append(each_line)    
                    else:
